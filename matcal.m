...
if plotme == 1
    ...
    if exist('OCTAVE_VERSION', 'builtin') ~= 0 % very simple output plot for octave users
    ...
    else % otherwise continue with more fancy plot
        ...
        % MatCal version and cal curve used
        ...
        % 14C date input and outputted cal ranges
        ageboxstr = ['^1^4C date: ',num2str(c14ageorig),' \pm ',num2str(c14errorig),' ^1^4C yr BP'];
        ...
        elseif (c14age + 4*c14err) > max(curve14c) || (c14age - 4*c14err) < min(curve14c) % 4 sigma possibly overkill
            title('Warning! Age calibration may exceed limits of calibration curve.')
        end
